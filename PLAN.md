...
- **Override-aware resolution:** markers resolve against the *current* tree, not just the raw marker text. The parser takes the conversation's stored `overrides` and: (a) resolves a path segment by first looking among the parent's **effective children** (by `override.parentId ?? marker parent`) for one whose **effective title** (`override.title ?? marker title`) matches, then falling back to the name-path hash; (b) makes `/up` and `/sibling` climb the **effective parent**. So a UI rename (A→APPLE) is addressable as `/node APPLE > B`, and `/up` follows UI drag-reparents. Identity (the hash id) is unchanged, so the original name/path still re-enters the same node (no split/duplicate).
- **Edge cases:**
...
- **Highlight/scroll for virtualized messages (Bug C) — DONE:** highlights are now **sticky** (re-applied as off-screen turns render in, via `dom-mapper.onRecorrelate`), and clicking a node whose first message isn't rendered **hunts** — pages the conversation toward it, re-correlates, then scrolls it precisely into view (`highlighting.js`). May need minor in-browser tuning of the page step / delay.
- **Override-aware marker resolution (Bug A / naming) — DONE:** the parser now takes `overrides` and resolves against the current tree (effective titles + effective parents), so `/node APPLE > B` works after a rename and `/up` follows drag-reparents (see §5). Identity unchanged. Remaining edge: a *forward reference* (addressing a node by its new-parent path before its creating marker appears) makes a new node — rare, documented.
...
- **Multi-chatbot support (v2 — consider):** the analysis layer (`parser.js`, `merge.js`) is already chatbot-agnostic — its only input contract is `{ uuid, role, text, index }` — and rendering (`tree-panel.js`) is nearly so. The Claude-specific knowledge lives in the **extraction** layer, but it isn't yet behind a single seam: it's spread across `interceptor.js` (endpoint regex), `api-client.js` (org/conversation URL shapes, `normalize()` field names like `chat_messages`/`sender`/`content[]`), `dom-mapper.js` (`TURN_SELECTOR`, user-message selector), one selector in `highlighting.js`, and `manifest.json` matches. To support ChatGPT/Gemini/etc., introduce an **adapter interface** — `{ matches(url), isConversationEndpoint(url), normalize(body) → messages[], dom: { turnSelector, userMessageSelector } }` — selected by hostname at boot, and move those constants into per-bot adapter files (`adapters/claude.js`, …). Note extraction here is really two parts: a **network-payload** adapter and a **DOM-correlation** adapter (we read messages from the API response, not by scraping rendered HTML). Some chatbots may lack a clean conversation-fetch endpoint and force DOM-only extraction; keep `normalize()` agnostic about whether messages originated from network or DOM so those collapse cleanly into one adapter. The parser and merge layers should not need to change.
